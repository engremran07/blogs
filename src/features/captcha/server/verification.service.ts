...
export class CaptchaVerificationService {
  ...
  constructor(
    ...
    this.secrets = {
      turnstileSecret: secrets?.turnstileSecret ?? process.env.CLOUDFLARE_TURNSTILE_SECRET,
      recaptchaSecret: secrets?.recaptchaSecret ?? process.env.RECAPTCHA_SECRET_KEY,
      recaptchaV2Secret: secrets?.recaptchaV2Secret ?? process.env.RECAPTCHA_V2_SECRET_KEY ?? process.env.RECAPTCHA_SECRET_KEY,
      hcaptchaSecret: secrets?.hcaptchaSecret ?? process.env.HCAPTCHA_SECRET_KEY,
    };
  }
  ...
  private async verifyTurnstile(token: string, clientIp: string): Promise<CaptchaVerificationResult> {
    ...
    return {
      ...
      provider: 'turnstile',
      error: data.success ? undefined : `Turnstile: ${(data['error-codes'] ?? []).join(', ')}`,
    };
  }

  private async verifyRecaptchaV3(token: string, clientIp: string): Promise<CaptchaVerificationResult> {
    const secret = this.secrets.recaptchaSecret;
    if (!secret) {
      return { success: false, error: 'reCAPTCHA v3 secret key not configured' };
    }
    ...
    return {
      ...
      score: data.score,
      error: passed ? undefined : `reCAPTCHA v3: score ${data.score ?? 'N/A'} below threshold`,
    };
  }

  private async verifyRecaptchaV2(token: string, clientIp: string): Promise<CaptchaVerificationResult> {
    const secret = this.secrets.recaptchaV2Secret;
    if (!secret) {
      return { success: false, error: 'reCAPTCHA v2 secret key not configured' };
    }
    ...
    return {
      ...
      error: data.success ? undefined : `reCAPTCHA v2: ${(data['error-codes'] ?? []).join(', ')}`,
    };
  }

  private async verifyHCaptcha(token: string, clientIp: string): Promise<CaptchaVerificationResult> {
    ...
    return {
      success: data.success === true,
      provider: 'hcaptcha',
      error: data.success ? undefined : `hCaptcha: ${(data['error-codes'] ?? []).join(', ')}`,
    };
  }
   ...
   */
  private async verifyInhouse(token: string, captchaId?: string): Promise<CaptchaVerificationResult> {
    if (!captchaId) {
    ...
    try {
      ...
      if (!challenge) {
        return { success: false, error: 'In-house captcha: challenge not found' };
      }

      // Check expiry
      if (new Date() > new Date(challenge.expiresAt)) {
        // Clean up expired challenge
        await this.prisma.captchaChallenge?.delete?.({ where: { id: captchaId } }).catch(() => {});
        return { success: false, error: 'In-house captcha: challenge expired' };
      }

      // Check if already used
      if (challenge.usedAt) {
        return { success: false, error: 'In-house captcha: challenge already used' };
      }

      // Check max attempts
      if (challenge.attempts >= (challenge.maxAttempts ?? 3)) {
        await this.prisma.captchaChallenge?.delete?.({ where: { id: captchaId } }).catch(() => {});
        return { success: false, error: 'In-house captcha: max attempts exceeded' };
      }
      ...
      if (expected.length !== submitted.length) {
        return { success: false, provider: 'custom', error: 'In-house captcha: incorrect answer' };
      }
  ...
  private async verifyAutoDetect(
    ...
    return { success: false, error: 'No CAPTCHA provider could verify the token' };
  }
}
