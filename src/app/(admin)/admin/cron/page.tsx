...
const TASK_META: Record<
  string,
  { label: string; description: string; gate: string }
> = {
  ...
  "release-stale-post-locks": {
    ...
    description:
      "Unlocks posts that have been locked for too long without activity",
    gate: "Always enabled",
  },
  "release-stale-page-locks": {
    label: "Release Stale Page Locks",
    description:
      "Unlocks pages that have been locked for too long without activity",
    gate: "Always enabled",
  },
  ...
  "sync-ad-slot-page-types": {
    ...
    description:
      "Adds new page types and removes deleted content from ad slots",
    gate: "adsEnabled",
  },
...
export default function CronAdminPage() {
    ...
    async (page: number) => {
      ...
      try {
        const res = await fetch(
          `/api/cron/history?page=${page}&limit=${ADMIN_PAGE_SIZE}`,
          {
  ...
  const triggerCron = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok && ![401, 503, 409].includes(res.status)) {
      ...
      if (res.status === 409) {
        toast(
          "Another cron execution is in progress. Try again later.",
          "error",
        );
        return;
      }
    ...
    <div>
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
            {TASK_NAMES.length} automated tasks &middot; Triggered via Vercel
            Cron or manually
            {disabledTaskCount > 0 && (
              <span className="ml-2 text-red-500 dark:text-red-400">
                &middot; {disabledTaskCount} task
                {disabledTaskCount > 1 ? "s" : ""} disabled by module settings
              </span>
            )}
          </p>
        </div>
      ...
      <div className="mb-6 rounded-xl border border-gray-200 bg-white p-5 dark:border-gray-700 dark:bg-gray-800">
        ...
        <p className="mt-2 text-xs text-gray-400 dark:text-gray-500">
          ...
          with your secret. Results appear below. History is persisted to the
          database.
        </p>
      </div>
        ...
        <div className="mb-6 rounded-xl border border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-800">
          ...
          <div className="divide-y divide-gray-50 dark:divide-gray-700/50">
            {latestRun.results.map((r) => {
              ...
              return (
                <div key={r.task} className="flex items-center gap-3 px-5 py-3">
      ...
      <div className="rounded-xl border border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-800">
        ...
        <div className="divide-y divide-gray-50 dark:divide-gray-700/50">
          {TASK_NAMES.map((name) => {
            ...
            const latestResult = latestRun?.results.find(
              (r) => r.task === name,
            );
            return (
              <div
        ...
        <div className="flex items-start gap-3 text-sm">
          ...
          <div className="text-blue-700 dark:text-blue-300">
            ...
            <p className="mt-1 text-blue-600 dark:text-blue-400 text-xs">
              In production, these tasks run automatically via Vercel Cron
              (configured in{" "}
              <code className="rounded bg-blue-100 px-1 dark:bg-blue-900/50">
                vercel.json
              </code>
              ). Each task checks its feature&apos;s kill switch in Site
              Settings before executing. A distributed lock prevents concurrent
              executions. Each task has a 30-second timeout. All runs are
              recorded to the database for auditing — view them in the Execution
