...
import {
  ...
  ChevronDown, Archive, FileText, Send, MoreHorizontal, Eye,
} from "lucide-react";
...
export default function AdminPagesPage() {
  ...
  async function handleDelete() {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) { toast("Page deleted", "success"); fetchPages(); }
  ...
  async function executeBulkAction() {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) { toast(data.message, "success"); setSelected(new Set()); fetchPages(); }
    ...
    <div>
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">{totalCount} pages</p>
        </div>
          ...
          <div className="relative" ref={bulkMenuRef}>
            ...
            {bulkMenuOpen && (
              <div className="absolute right-0 top-full z-20 mt-1 w-48 rounded-lg border border-gray-200 bg-white py-1 shadow-lg dark:border-gray-700 dark:bg-gray-800">
                <button onClick={() => setBulkConfirm({ action: "publish", label: "publish" })} className="flex w-full items-center gap-2 px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-700"><Send className="h-4 w-4 text-green-500" /> Publish</button>
                <button onClick={() => setBulkConfirm({ action: "draft", label: "move to draft" })} className="flex w-full items-center gap-2 px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-700"><FileText className="h-4 w-4 text-gray-500" /> Move to Draft</button>
                <button onClick={() => setBulkConfirm({ action: "archive", label: "archive" })} className="flex w-full items-center gap-2 px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-700"><Archive className="h-4 w-4 text-yellow-500" /> Archive</button>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200 bg-gray-50 text-left dark:border-gray-700 dark:bg-gray-800/80">
                ...
                <th className="px-4 py-3 font-medium text-gray-600 dark:text-gray-400">Template</th>
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
              {loading
                ...
                : pages.map((pg) => (
                    <tr key={pg.id} className={`hover:bg-gray-50 dark:hover:bg-gray-700/30 ${selected.has(pg.id) ? "bg-blue-50 dark:bg-blue-900/10" : ""}`}>
                      <td className="px-3 py-3">
                        <button onClick={() => toggleSelect(pg.id)} className="text-gray-500 hover:text-gray-700 dark:hover:text-gray-300">
                          {selected.has(pg.id) ? <CheckSquare className="h-4 w-4 text-blue-600" /> : <Square className="h-4 w-4" />}
                        </button>
                      </td>
                      <td className="px-4 py-3 font-medium text-gray-900 dark:text-white">
                        {pg.title}
                        {pg.isSystem && <span className="ml-2 text-xs text-gray-400">(System)</span>}
                      </td>
                      <td className="px-4 py-3 text-gray-500 dark:text-gray-400">/{pg.slug}</td>
                      <td className="px-4 py-3">
                        <span className={`inline-block rounded px-2 py-0.5 text-xs font-medium ${statusColors[pg.status] || ""}`}>{pg.status}</span>
                      </td>
                      <td className="px-4 py-3 text-gray-500 dark:text-gray-400">{pg.template}</td>
                      <td className="px-4 py-3 text-gray-500 dark:text-gray-400 whitespace-nowrap">{new Date(pg.createdAt).toLocaleDateString()}</td>
                      <td className="px-4 py-3">
      ...
      <ConfirmDialog open={!!bulkConfirm} onClose={() => { setBulkConfirm(null); setBulkMenuOpen(false); }} onConfirm={executeBulkAction} title={`Bulk ${bulkConfirm?.label || ""}`} message={`${bulkConfirm?.label} ${selected.size} page(s)?`} confirmText={bulkConfirm?.action === "delete" ? "Delete All" : "Confirm"} variant={bulkConfirm?.action === "delete" ? "danger" : "primary"} />
    </div>
  );
}
