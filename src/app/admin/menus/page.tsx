...
export default function AdminMenusPage() {
  ...
  async function loadMenus() {
    try {
      ...
      const siteMenus: MenuData[] = data.data?.menus || [];
        ...
        const defaults: MenuData[] = [
          {
            ...
            enabled: true,
            items: [
              { id: "h1", label: "Home", url: "/", target: "_self", parentId: null, order: 0, visible: true },
              { id: "h2", label: "Blog", url: "/blog", target: "_self", parentId: null, order: 1, visible: true },
              { id: "h3", label: "Tags", url: "/tags", target: "_self", parentId: null, order: 2, visible: true },
              { id: "h4", label: "About", url: "/about", target: "_self", parentId: null, order: 3, visible: true },
              { id: "h5", label: "Contact", url: "/contact", target: "_self", parentId: null, order: 4, visible: true },
            ],
          },
          {
            id: "footer",
            ...
            enabled: true,
            items: [
              { id: "f1", label: "Privacy Policy", url: "/privacy", target: "_self", parentId: null, order: 0, visible: true },
              { id: "f2", label: "Terms of Service", url: "/terms", target: "_self", parentId: null, order: 1, visible: true },
              { id: "f3", label: "Contact Us", url: "/contact", target: "_self", parentId: null, order: 2, visible: true },
            ],
          },
          {
            id: "topbar",
  ...
  const addMenu = useCallback(() => {
    ...
    setMenus((prev) => [...prev, { id, name: newMenuName, slot: newMenuSlot, enabled: true, items: [] }]);
    setActiveMenuId(id);
    setNewMenuName("");
  ...
  const deleteMenu = useCallback((menuId: string) => {
    ...
    setActiveMenuId((prev) => (prev === menuId ? menus[0]?.id || null : prev));
  }, [menus]);
  ...
  const addMenuItem = useCallback(() => {
    ...
    setMenus((prev) =>
      prev.map((m) =>
        m.id === activeMenuId
          ? { ...m, items: [...m.items, { id, label: "New Link", url: "/", target: "_self", parentId: null, order: m.items.length, visible: true }] }
          : m,
      ),
    );
  }, [activeMenuId]);
  ...
  const updateMenuItem = useCallback((menuId: string, itemId: string, field: string, value: string | boolean) => {
    setMenus((prev) =>
      prev.map((m) =>
        m.id === menuId
          ? { ...m, items: m.items.map((item) => (item.id === itemId ? { ...item, [field]: value } : item)) }
          : m,
      ),
    );
  }, []);
  ...
  const deleteMenuItem = useCallback((menuId: string, itemId: string) => {
    setMenus((prev) =>
      prev.map((m) => (m.id === menuId ? { ...m, items: m.items.filter((item) => item.id !== itemId) } : m)),
    );
  }, []);

  const toggleMenuEnabled = useCallback((menuId: string) => {
    setMenus((prev) => prev.map((m) => (m.id === menuId ? { ...m, enabled: !m.enabled } : m)));
  }, []);
        ...
        <div className="space-y-3">
          ...
          {menus.map((menu) => (
            ...
            >
              ...
              <span className={`h-2 w-2 rounded-full ${menu.enabled ? "bg-green-500" : "bg-gray-300"}`} />
            </button>
          ))}
          ...
          {showAddMenu ? (
            <div className="space-y-2 rounded-lg border border-gray-200 bg-white p-3 dark:border-gray-700 dark:bg-gray-800">
              ...
              />
              <Select value={newMenuSlot} onChange={(e) => setNewMenuSlot(e.target.value)}>
              ...
              <div className="flex gap-2">
                <Button size="sm" onClick={addMenu} disabled={!newMenuName.trim()}>
                  Add
                </Button>
                <Button size="sm" variant="ghost" onClick={() => setShowAddMenu(false)}>
                  Cancel
                </Button>
              </div>
            </div>
        ...
        <div className="lg:col-span-3">
          {activeMenu ? (
            <div className="rounded-xl border border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-800">
              ...
              <div className="flex items-center justify-between border-b border-gray-200 p-4 dark:border-gray-700">
                ...
                <div className="flex items-center gap-2">
                  ...
                  >
                    {activeMenu.enabled ? <Eye className="h-4 w-4" /> : <EyeOff className="h-4 w-4" />}
                    ...
                    {activeMenu.items.map((item) => (
                      ...
                      >
                        ...
                        <div className="grid flex-1 gap-2 sm:grid-cols-3">
                          <input
                            value={item.label}
                            onChange={(e) => updateMenuItem(activeMenu.id, item.id, "label", e.target.value)}
                            placeholder="Label"
                          ...
                          <input
                            value={item.url}
                            onChange={(e) => updateMenuItem(activeMenu.id, item.id, "url", e.target.value)}
                            placeholder="URL"
                            className="rounded-md border border-gray-300 bg-white px-2.5 py-1.5 text-sm focus:border-blue-500 focus:outline-none dark:border-gray-600 dark:bg-gray-700 dark:text-white"
                          />
                          <div className="flex items-center gap-2">
                            <select
                              value={item.target}
                              onChange={(e) => updateMenuItem(activeMenu.id, item.id, "target", e.target.value)}
                            ...
                            <button
                              onClick={() => updateMenuItem(activeMenu.id, item.id, "visible", !item.visible)}
                            ...
                            <button
                              onClick={() => deleteMenuItem(activeMenu.id, item.id)}
            ...
            <div className="flex h-64 items-center justify-center rounded-xl border border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-800">
              <p className="text-gray-500 dark:text-gray-400">Select or create a menu to start editing</p>
            </div>
          )}
        </div>
      </div>
