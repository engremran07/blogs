...
export default function CategoriesPage() {
  ...
  function flattenTree(cats: Category[], parentId: string | null = null): { id: string; parentId: string | null; sortOrder: number }[] {
  ...
  function removeCatFromTree(cats: Category[], id: string): { tree: Category[]; removed: Category | null } {
  ...
  function insertCatInTree(
    ...
    if (position === "child") {
      return cats.map((cat) => {
        if (cat.id === targetId) {
          return { ...cat, children: [...(cat.children || []), { ...category, children: category.children || [] }] };
        }
        if (cat.children?.length) {
          return { ...cat, children: insertCatInTree(cat.children, targetId, position, category) };
        }
        return cat;
      });
    }
    ...
    for (const cat of cats) {
      ...
      if (cat.children?.length) {
        result.push({ ...cat, children: insertCatInTree(cat.children, targetId, position, category) });
      } else {
        result.push(cat);
      }
  ...
  function isDescendant(cats: Category[], parentId: string, childId: string): boolean {
    for (const cat of cats) {
      if (cat.id === parentId) {
        return containsCat(cat.children || [], childId);
      }
      if (cat.children?.length && isDescendant(cat.children, parentId, childId)) return true;
    }
    return false;
  }
  ...
  function moveCat(id: string, direction: "up" | "down") {
    function moveInList(cats: Category[]): Category[] {
      ...
      return cats.map((cat) =>
        cat.children?.length ? { ...cat, children: moveInList(cat.children) } : cat,
      );
    }
  ...
  function makeChild(id: string) {
    ...
    if (!newParent) return;
    const newTree = insertCatInTree(treeWithout, newParent.id, "child", removed);
    setTree(newTree);
  ...
  function countAll(cats: Category[]): number {
    let c = cats.length;
    for (const cat of cats) if (cat.children?.length) c += countAll(cat.children);
    return c;
  }
    ...
    return (
      <div key={cat.id}>
        ...
        {isDropTarget && dropPos === "before" && (
          <div className="mx-3 h-0.5 rounded bg-blue-500" style={{ marginLeft: depth > 0 ? `${depth * 24 + 12}px` : undefined }} />
        )}
        ...
        {/* Children */}
        {isExpanded && cat.children?.map((child) => renderNode(child, depth + 1))}
      </div>
    );
  }
      ...
      >
        ...
        {(deleteCat?.postCount ?? 0) > 0 && (
          <p className="mt-2 text-sm text-amber-600 dark:text-amber-400">
            ⚠ This category has {deleteCat?.postCount} assigned posts. They
