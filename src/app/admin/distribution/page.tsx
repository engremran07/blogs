...
interface Stats {
  ...
  successRate: number;
  platformHealth: Record<string, { status: string; circuitOpen: boolean; rateLimited: boolean }>;
}
...
const PLATFORMS = [
  ...
  "pinterest", "reddit", "instagram", "tiktok", "medium",
  "youtube", "custom",
];
...
export default function DistributionAdminPage() {
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      const [chRes, recRes, statRes] = await Promise.all([
        ...
        fetch("/api/distribution/records").then((r) => r.json()),
        fetch("/api/distribution/stats").then((r) => r.json()),
      ]);
      if (chRes.success) setChannels(chRes.data);
      if (recRes.success) setRecords(Array.isArray(recRes.data) ? recRes.data : recRes.data?.data ?? []);
  ...
  async function saveChannel() {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        toast(editingChannel ? "Channel updated" : "Channel created", "success");
  ...
  async function handleDelete() {
    if (!deleteId) return;
    try {
      const res = await fetch(`/api/distribution/channels/${deleteId}`, { method: "DELETE" });
      const data = await res.json();
      if (data.success) {
  ...
  async function validateChannel(id: string) {
    try {
      const res = await fetch(`/api/distribution/channels/${id}/validate`, { method: "POST" });
      const data = await res.json();
  ...
  function statusBadge(status: string) {
    const map: Record<string, { bg: string; text: string; icon: React.ReactNode }> = {
      PUBLISHED: { bg: "bg-green-100 dark:bg-green-900/30", text: "text-green-700 dark:text-green-400", icon: <CheckCircle className="h-3.5 w-3.5" /> },
      FAILED: { bg: "bg-red-100 dark:bg-red-900/30", text: "text-red-700 dark:text-red-400", icon: <XCircle className="h-3.5 w-3.5" /> },
      PENDING: { bg: "bg-amber-100 dark:bg-amber-900/30", text: "text-amber-700 dark:text-amber-400", icon: <Clock className="h-3.5 w-3.5" /> },
      SCHEDULED: { bg: "bg-blue-100 dark:bg-blue-900/30", text: "text-blue-700 dark:text-blue-400", icon: <Clock className="h-3.5 w-3.5" /> },
      PUBLISHING: { bg: "bg-indigo-100 dark:bg-indigo-900/30", text: "text-indigo-700 dark:text-indigo-400", icon: <Send className="h-3.5 w-3.5" /> },
      CANCELLED: { bg: "bg-gray-100 dark:bg-gray-700", text: "text-gray-600 dark:text-gray-400", icon: <XCircle className="h-3.5 w-3.5" /> },
      RATE_LIMITED: { bg: "bg-orange-100 dark:bg-orange-900/30", text: "text-orange-700 dark:text-orange-400", icon: <AlertTriangle className="h-3.5 w-3.5" /> },
    };
  ...
  function platformIcon(platform: string) {
    ...
    return (
      <span className={`inline-flex items-center rounded px-2 py-0.5 text-xs font-medium ${colors[platform] ?? "bg-gray-100 text-gray-700 dark:bg-gray-700 dark:text-gray-400"}`}>
        {platform}
      </span>
    );
  }
  ...
  return (
    <div>
      ...
      {tab === "overview" && (
        <div className="space-y-6">
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {[
              { label: "Total", value: stats?.records.total ?? 0, icon: <Send className="h-5 w-5" />, color: "blue" },
              { label: "Success", value: stats?.records.published ?? 0, icon: <CheckCircle className="h-5 w-5" />, color: "green" },
              { label: "Failed", value: stats?.records.failed ?? 0, icon: <XCircle className="h-5 w-5" />, color: "red" },
              { label: "Pending", value: stats?.records.pending ?? 0, icon: <Clock className="h-5 w-5" />, color: "amber" },
            ].map((s) => (
              <div key={s.label} className="rounded-xl border border-gray-200 bg-white p-5 dark:border-gray-700 dark:bg-gray-800">
                <div className="flex items-center justify-between">
                  <p className="text-sm font-medium text-gray-500 dark:text-gray-400">{s.label}</p>
                  <span className={`text-${s.color}-600 dark:text-${s.color}-400`}>{s.icon}</span>
                </div>
          ...
          {/* Platform Health */}
          {stats?.platformHealth && Object.keys(stats.platformHealth).length > 0 && (
            <div className="rounded-xl border border-gray-200 bg-white p-5 dark:border-gray-700 dark:bg-gray-800">
              <h3 className="mb-4 font-semibold text-gray-900 dark:text-white">Platform Health</h3>
              <div className="space-y-3">
                {Object.entries(stats.platformHealth).map(([platform, health]) => (
                  <div key={platform} className="flex items-center justify-between rounded-lg bg-gray-50 px-4 py-3 dark:bg-gray-700/50">
                    <div className="flex items-center gap-3">
                    ...
                    <div className="flex items-center gap-4">
                      <span className={`text-sm font-medium ${
                        ...
                        : health.status === "circuit_open" ? "text-red-600 dark:text-red-400"
                        : health.status === "rate_limited" ? "text-orange-600 dark:text-orange-400"
          ...
          {/* Channels list */}
          <div className="rounded-xl border border-gray-200 bg-white p-5 dark:border-gray-700 dark:bg-gray-800">
            <h3 className="mb-4 font-semibold text-gray-900 dark:text-white">Channels</h3>
            {channels.length === 0 ? (
              <p className="text-sm text-gray-400">No channels configured yet</p>
            ) : (
              <div className="space-y-2">
                {channels.map((ch) => (
                  <div key={ch.id} className="flex items-center justify-between rounded-lg bg-gray-50 px-4 py-3 dark:bg-gray-700/50">
                    <div className="flex items-center gap-3">
                      <span className={`h-2.5 w-2.5 rounded-full ${ch.enabled ? "bg-green-500" : "bg-gray-400"}`} />
                      <div>
      ...
      {tab === "channels" && (
        <div>
          <div className="mb-4 flex justify-end">
            <Button onClick={openCreate} icon={<Plus className="h-4 w-4" />}>New Channel</Button>
          </div>
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-gray-700/50">
                {channels.map((ch) => (
                  <tr key={ch.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/30">
                    <td className="px-4 py-3 font-medium text-gray-900 dark:text-white">{ch.name}</td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex justify-end gap-1">
                        <button onClick={() => validateChannel(ch.id)} title="Validate" className="rounded p-1.5 text-gray-400 hover:bg-green-50 hover:text-green-600 dark:hover:bg-green-900/30 dark:hover:text-green-400">
                          <CheckCircle className="h-4 w-4" />
                        </button>
                        <button onClick={() => openEdit(ch)} className="rounded p-1.5 text-gray-400 hover:bg-gray-100 hover:text-gray-700 dark:hover:bg-gray-700 dark:hover:text-gray-300">
                          <Edit2 className="h-4 w-4" />
                        </button>
                        <button onClick={() => setDeleteId(ch.id)} className="rounded p-1.5 text-gray-400 hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-900/30 dark:hover:text-red-400">
                ...
                {channels.length === 0 && (
                  <tr><td colSpan={5} className="px-4 py-8 text-center text-gray-400">No channels configured</td></tr>
                )}
              </tbody>
            </table>
          </div>
      ...
      {tab === "records" && (
        <div>
          <div className="mb-4 flex justify-end">
            <Button variant="outline" onClick={fetchAll} icon={<RefreshCw className="h-4 w-4" />}>Refresh</Button>
          </div>
          <div className="rounded-xl border border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-800">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-200 dark:border-gray-700">
                  <th className="px-4 py-3 text-left font-medium text-gray-500 dark:text-gray-400">Post</th>
                  <th className="px-4 py-3 text-left font-medium text-gray-500 dark:text-gray-400">Channel</th>
                  <th className="px-4 py-3 text-left font-medium text-gray-500 dark:text-gray-400">Status</th>
                  ...
                  <tr key={r.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/30">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-gray-600 dark:text-gray-400">{r.channel?.name ?? r.channelId}</span>
                      </div>
                    </td>
                    <td className="px-4 py-3">{statusBadge(r.status)}</td>
                    <td className="px-4 py-3 text-gray-600 dark:text-gray-400">{r.retryCount}</td>
                    <td className="px-4 py-3 text-xs text-gray-500 dark:text-gray-400">
                      {new Date(r.createdAt).toLocaleDateString()}
                    </td>
                    <td className="px-4 py-3">
                      {r.externalUrl ? (
                        <a href={r.externalUrl} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline dark:text-blue-400 text-xs">
                          View
                        </a>
                      ) : r.error ? (
                        <span className="text-xs text-red-500 dark:text-red-400 truncate max-w-40 block" title={r.error}>
                          {r.error}
                        </span>
                      ) : (
      ...
      {/* ─── Channel Modal ─── */}
      <Modal open={channelModal} onClose={() => setChannelModal(false)} title={editingChannel ? "Edit Channel" : "New Channel"} size="lg">
        <div className="space-y-4">
          {/* Row 1: Name + Platform */}
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <Input label="Name" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="Channel name" />
          ...
          <div className="grid grid-cols-2 gap-x-6 gap-y-2 sm:grid-cols-3">
            {[
              ...
              { key: "autoPublish" as const, label: "Auto-publish on post publish" },
            ].map((opt) => (
          ...
          <div className="flex justify-end gap-2 pt-2">
            <Button variant="outline" onClick={() => setChannelModal(false)}>Cancel</Button>
            <Button onClick={saveChannel} loading={saving}>{editingChannel ? "Update" : "Create"}</Button>
          </div>
        </div>
      </Modal>

      {/* ─── Delete Confirmation ─── */}
      <Modal open={!!deleteId} onClose={() => setDeleteId(null)} title="Delete Channel">
        <p className="text-sm text-gray-600 dark:text-gray-400">
          Are you sure you want to delete <strong className="text-gray-900 dark:text-white">{deleteCh?.name}</strong>?
