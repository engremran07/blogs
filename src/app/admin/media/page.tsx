...
import type { MediaItem, UpdateMediaInput, MediaFilter, MediaSort, PaginatedResult, MediaFolder, MediaSeoAuditResult } from "@/features/media/types";
  ...
  return (
    <div className="flex h-[calc(100vh-12rem)] items-center justify-center rounded-xl border border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-800">
      <div className="flex flex-col items-center gap-3">
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400">Loading Media Manager…</p>
      </div>
    </div>
  );
}
...
export default function AdminMediaPage() {
  ...
  const handleUploadFromUrl = useCallback(
    async (url: string, meta?: Partial<UpdateMediaInput>): Promise<MediaItem> => {
      ...
      const json = await res.json();
      if (!json.success) throw new Error(json.error || "Upload from URL failed");
      return json.data;
    },
    []
  );
  ...
  const handleAuditSeo = useCallback(
    async (item: MediaItem): Promise<MediaSeoAuditResult> => {
      ...
      if (!hasAltText) suggestions.push("Add descriptive alt text for accessibility and SEO");
